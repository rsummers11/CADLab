...
%     [X Y Z] = ndgrid(xs,-ys,zs);
    ...
    if use_vtk_indixing
        fv.vertices(:,1) = (fv.vertices(:,1)-1)*dim(1)*RD;  % VTK indexing starting from 0.0 mm
        fv.vertices(:,2) = (fv.vertices(:,2)-1)*dim(2)*RD;  % VTK indexing starting from 0.0 mm
        fv.vertices(:,3) = (fv.vertices(:,3)-1)*dim(3)*RD;  % VTK indexing starting from 0.0 mm 
    else
        fv.vertices(:,1) = fv.vertices(:,1)*dim(1)*RD;  % VTK indexing starting from 0.0 mm
        fv.vertices(:,2) = fv.vertices(:,2)*dim(2)*RD;  % VTK indexing starting from 0.0 mm
