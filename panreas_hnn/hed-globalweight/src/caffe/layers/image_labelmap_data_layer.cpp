...
#include "shared.h" //Holger

double shared::posFraction = -1.0;
...
void ImageLabelmapDataLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  // Holger: compute global balancing weight from images (WARNING: Assumes training images are read first!)
  if (shared::posFraction < 0.0)
  {
	  unsigned long posCount = 0;
	  unsigned long totalCount = 0;
	  for (unsigned int i=0; i<lines_.size(); i++)
	  {
	    cv::Mat cv_gt = ReadImageToCVMat(root_folder + lines_[i].second,
		                            new_height, new_width, 0);    
	    cv_gt = cv_gt/255; // assume image has 255 as positive pixel value
	    //std::cout << "rows: " << cv_gt.rows << std::endl; 
	    //std::cout << "cols: " << cv_gt.cols << std::endl; 
	    //std::cout << "sum: " << cv::sum(cv_gt)[0] << ", total: " << cv_gt.rows*cv_gt.cols << std::endl; 
	    posCount += cv::sum(cv_gt)[0];
	    totalCount += cv_gt.rows*cv_gt.cols;
	  }
	  std::cout << "posCount: " << posCount << std::endl; 
	  std::cout << "totalCount: " << totalCount << std::endl; 
	  shared::posFraction = (double)posCount/(double)totalCount; // define posFraction
	  std::cout << "shared::posFraction: " << shared::posFraction << " =^ " << 100*shared::posFraction << "%" << std::endl; 
  }
