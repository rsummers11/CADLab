-- adapted from https://github.com/torch/nn/blob/master/SpatialContrastiveNormalization.lua

local SpatialContrastiveNormalization__ = torch.class('SpatialContrastiveNormalization__')
-- local SpatialContrastiveNormalization = torch.class 'SpatialContrastiveNormalization'  
-- local SpatialContrastiveNormalization = torch.class('nn.SpatialContrastiveNormalization','nn.Module')

function SpatialContrastiveNormalization__:__init(nInputPlane, kernel, threshold, thresval)
   -- parent.__init(self)
...
function SpatialContrastiveNormalization__:updateOutput(input)
   self.output = self.normalizer:forward(input)
   return self.output
end

function SpatialContrastiveNormalization__:updateGradInput(input, gradOutput)
