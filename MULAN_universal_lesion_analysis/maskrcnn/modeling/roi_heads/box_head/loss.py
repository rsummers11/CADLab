...
from maskrcnn.modeling.balanced_positive_negative_sampler import (
...
class FastRCNNLossComputation(object):
    ...
    def prepare_targets(self, proposals, targets):
        ...
        for proposals_per_image, targets_per_image in zip(proposals, targets):
            if len(targets_per_image) > 0:
                ...
                if cfg.DATALOADER.NEG_SLICE_IN_TRAIN == 0:
                    print('one slice without objects')
                num_boxes = len(proposals_per_image)
                device = proposals_per_image.bbox.device
                labels_per_image = torch.zeros(num_boxes, dtype=torch.int64).to(device)  # no gt boxes, so all proposals should be false
                regression_targets_per_image = torch.zeros(num_boxes, 4, dtype=torch.float32).to(device)
                matched_idxs_per_image = -torch.ones(num_boxes, dtype=torch.int64).to(device)
    ...
    def __call__(self, class_logits, box_regression):
        ...
        if not cfg.MODEL.ROI_BOX_HEAD.FOCAL_LOSS:
            classification_loss = F.cross_entropy(class_logits, labels)
        else:
            alpha = .5
            gamma = 2

            p = class_logits[:,1].sigmoid()
            ...
            w = valid_mask * (alpha*labels_float + (1-alpha)*(1-labels_float))  # w = alpha if t > 0 else 1-alpha
            classification_loss = w * (1-pt).pow(gamma) * pt.log()
            classification_loss = -classification_loss.sum() / (labels_float*valid_mask).sum()
