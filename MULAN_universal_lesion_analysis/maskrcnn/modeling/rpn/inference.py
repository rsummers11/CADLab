...
from maskrcnn.structures.boxlist_ops import remove_small_boxes
...
class RPNPostProcessor(torch.nn.Module):
    ...
    def add_gt_proposals(self, proposals, targets):
        ...
        for proposal, gt_box in zip(proposals, gt_boxes):
            ...
            # yk: add a field to indicate if a box is a proposal or gt
            is_gt = torch.zeros(proposal.bbox.shape[0], dtype=torch.uint8, device=device)
            if len(gt_box) > 0:
                is_gt[-gt_box.bbox.shape[0]:] = 1
            proposal.add_field("is_gt", is_gt)
            proposals_new.append(proposal)
    ...
    def forward(self, anchors, objectness, box_regression, targets=None):
        ...
        if (self.training and targets is not None) \
            or cfg.TEST.EVAL_SEG_TAG_ON_GT:
...
def make_rpn_postprocessor(rpn_box_coder, is_train):
    ...
    if not is_train:
        pre_nms_top_n = cfg.MODEL.RPN.PRE_NMS_TOP_N_TEST
