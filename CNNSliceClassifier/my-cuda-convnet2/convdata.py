...
class ImageDataProvider(LabeledDataProvider):
    ...
    def get_plottable_data(self, data, add_mean=True):
        ...
        return n.require((data + (mean if add_mean else 0)).T.reshape(data.shape[1], self.num_colors, self.inner_size, self.inner_size).swapaxes(1,3).swapaxes(1,2) / 255.0, dtype=n.single)
...
class GeneralDataProvider(LabeledDataProvider):
    def __init__(self, data_dir, batch_range=None, init_epoch=1, init_batchnum=None, dp_params=None, test=False):
        LabeledDataProvider.__init__(self, data_dir, batch_range, init_epoch, init_batchnum, dp_params, test)
        self.img_size = self.batch_meta['img_size'] 
        self.num_colors = self.batch_meta['img_channels']
	print "GeneralDataProvider with image size = {}, and {} channel(s)...".format(self.img_size,self.num_colors)
        self.inner_size =  dp_params['inner_size'] if dp_params['inner_size'] > 0 else self.batch_meta['img_size']
        self.border_size = (self.img_size - self.inner_size) / 2
        self.multiview = dp_params['multiview_test'] and test
        self.num_views = 9
