...
import math as m
import layer as lay
from convdata import ImageDataProvider, CIFARDataProvider, GeneralDataProvider, DummyConvNetLogRegDataProvider
...
class FeatureWriterDriver(Driver):
	...
	print "writing features {} to {}:".format(self.convnet.write_features, self.convnet.feature_path)
	...
	print "num_ftrs: {}".format(self.num_ftrs)
...
class ConvNet(IGPUModel):
    ...
    def set_driver(self):
	if self.op.get_value('multiview_test') and self.op.get_value('write_features'):
	    print("[WARNING] multiview-test and write-features cannot be execuded at the same time!")
	    print("[WARNING] -> just execute multiview-test")
...
#                print self.layers[errname]['children']
    ...
    def get_options_parser(cls):
        ...
        DataProvider.register_data_provider('general', 'general data provider', GeneralDataProvider)
