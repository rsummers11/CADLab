...
# Description:
# Auxiliary file called by one_load_model_other_modalities.py.
...
def main(args, tokenizer, model):
    if os.path.isdir(args.result_root):
        args.result_root = args.result_root + f"/parsing_results_llm_{args.modality}.csv"
    report_label_file = args.result_root
    model_name = args.model

    labels_to_join = OrderedDict()
    if args.modality=='ct':
        label_set =  ['lung lesion', 'liver lesion', 'kidney lesion', 'adrenal gland abnormality','pleural effusion']
    elif args.modality=='mri':
        label_set=  ['liver lesion', 'kidney lesion', 'adrenal gland abnormality']
    elif args.modality=='pet':
        label_set =  ['hypermetabolic abnormality in the thorax', 'hypermetabolic abnormality in the abdomen', 'hypermetabolic abnormality in the pelvis']

    label_set_mimic_labels = label_set
    label_set_mimic_probability = label_set

    label_set_mimic_location = label_set
    label_set_mimic_severity = label_set

    primary_labels = {}
    for index_label, label in enumerate(label_set):
        labels_to_join[label] = [index_label]
    ...
    def parse_location(sentence_answer, fn_inputs):
        ...
        location_filter_prompt = Node([lambda sentence_, label_, expression_: f'''Consider in your answer: 1) medical wording synonyms, subtypes of abnormalities 2) abreviations of the medical vocabulary. Given the complete {args.modality} report "{sentence_}", is the isolated adjective "{expression_}", on its own, characterizing a medical finding in what way? Respond only with "_" where _ is the number corresponding to the correct answer.
...
Answer:"'''],[False,True], 3 , parse_location_filter)
    ...
    if args.single_file is not None:
        chexpert_df = pd.read_csv(args.single_file)
    ...
    location_node =  Node([lambda sentence_, label_: f'Given the complete {args.modality} report "{sentence_}", does it mention a location for specifically "{label_set_mimic_location[label_]}"? Respond only with "Yes" or "No".'],
    ['',Node([lambda sentence_, label_: f'Given the {args.modality} report "{sentence_}", list the localizing expressions characterizing specifically the "{label_set_mimic_location[label_]}" finding. Each adjective expression should be between quotes, broken down into each and every one of the localizing adjectives and each independent localiziation prepositional phrase, and separated by comma. Output an empty list ("[]" is an empty list) if there are 0 locations mentioned for "{label_set_mimic_location[label_]}". Do not mention the central nouns identified as "{label_set_mimic_location[label_]}". Do not mention any nouns that are not part of an adjective. Only include in your answer location adjectives adjacent to the mention of the "{label_set_mimic_location[label_]}" finding. Exclude from your answer adjectives for other findings. Use very short answers without complete sentences. Start the list (0+ elements) of only localizing adjectives or localizing expressions (preposition + noun) right here: ['],None, 200, parse_location)
    ])
    
    severity_node =  Node([lambda sentence_, label_: f'Given the complete {args.modality} report "{sentence_}", would you be able to characterize the severity of "{label_set_mimic_severity[label_]}", as either "Mild", "Moderate" or "Severe" only from the words of the report? Respond only with "Yes" or "No".'],
        [-1,Node([lambda sentence_, label_: f'Given the complete {args.modality} report "{sentence_}", characterize the severity of "{label_set_mimic_severity[label_]}" as either "Mild", "Moderate" or "Severe" or "Undefined" only from the words of the report, and not from comparisons or changes. Do not add extra words to your answer and exclusively use the words from one of those four options.'],None, 5, parse_severity)
        ])
    ...
    number_prompt =  Node([lambda sentence_, label_: f'{first_part}. Given the complete {args.modality} report "{sentence_}", estimate from the report wording how likely another radiologist is to observe the presence of any type of "{label_set_mimic_probability[label_]}" in the same imaging. Respond with the template "___% likely." and no other words.'],
        list(range(102)), 3, parse_percentage_number)
    
    number_prompt_present =  Node([lambda sentence_, label_: f'{first_part} Given the complete {args.modality} report "{sentence_}", consistent with the radiologist observing "{label_set_mimic_probability[label_]}", estimate from the report wording how likely another radiologist is to observe the presence of any type of "{label_set_mimic_probability[label_]}" in the same imaging. Respond with the template "___% likely." and no other words.'],
        list(range(102)), 4, parse_percentage_number)
    
    number_prompt_absent =  Node([lambda sentence_, label_: f'{first_part} Given the complete {args.modality} report "{sentence_}", consistent with the radiologist stating the absence of evidence "{label_set_mimic_probability[label_]}", estimate from the report wording how likely another radiologist is to observe the presence of any type of "{label_set_mimic_probability[label_]}" in the same imaging. Respond with the template "___% likely." and no other words.'],
        list(range(102)), 4, parse_percentage_number)

    inner_prompts = Node([lambda sentence_, label_:f'Given the full {args.modality} report "{sentence_}", use a one sentence logical deductive reasoning to infer if the radiologist characterized {"specifically" if label_ not in (heart_labels) else ""} "{label_set_mimic_labels[label_]}" as stable or unchanged. Respond only with "Yes" or "No".'],
                [Node([lambda sentence_, label_: f'Given the full {args.modality} report "{sentence_}", use a one sentence logical deductive reasoning to infer if the radiologist stated the absence of evidence of "{label_set_mimic_labels[label_]}". Respond only with "Yes" or "No".'],
                    [[-2, number_prompt, '', -1],
                    [0, number_prompt, '', -1]]),
                Node([lambda sentence_, label_: f'Given the full {args.modality} report "{sentence_}", use a one sentence logical deductive reasoning to infer if "{label_set_mimic_labels[label_]}" might be present. Respond only with "Yes" or "No".'],
                    [Node([lambda sentence_, label_: f'Given the full {args.modality} report "{sentence_}", use a one sentence logical deductive reasoning to infer if the radiologist characterized {"specifically" if label_ not in (heart_labels) else ""} "{label_set_mimic_labels[label_]}" as normal. Respond only with "Yes" or "No".'],
                        [[-3,101,'',-1],
    ...
    number_inner_prompts_solo = Node([lambda sentence_, label_:f'Given the full {args.modality} report "{sentence_}", use a one sentence logical deductive reasoning to infer if the radiologist characterized {"specifically" if label_ not in (heart_labels) else ""} "{label_set_mimic_probability[label_]}" as stable or unchanged. Respond only with "Yes" or "No".'],
            [number_prompt,
            Node([lambda sentence_, label_: f'Given the full {args.modality} report "{sentence_}", use a one sentence logical deductive reasoning to infer if "{label_set_mimic_probability[label_]}" might be present. Respond only with "Yes" or "No".'],
                [Node([lambda sentence_, label_: f'Given the full {args.modality} report "{sentence_}", use a one sentence logical deductive reasoning to infer if the radiologist characterized {"specifically" if label_ not in (heart_labels) else ""} "{label_set_mimic_probability[label_]}" as normal. Respond only with "Yes" or "No".'],
                    [101,
                    number_prompt_absent]
                    ),
                number_prompt_present]
            )])
    prompts = Node([lambda sentence_, label_: f'Given the full {args.modality} report "{sentence_}", use a one sentence logical deductive reasoning to infer if the radiologist observed possible presence of evidence of "{label_set_mimic_labels[label_]}". Respond only with "Yes" or "No".'],
            [inner_prompts,
            [Node([lambda sentence_, label_: f'Given the complete {args.modality} report "{sentence_}", consistent with the radiologist observing "{label_set_mimic_labels[label_]}", estimate from the report wording how likely another radiologist is to observe "{label_set_mimic_labels[label_]}" in the same imaging. Respond with the template "___% likely." and no other words.'],
                [-1,
    ...
    def run_one_report(idx, row, model_list):
        ...
        subjectid_studyid = row['subjectid_studyid']
        if f"{row['subjectid_studyid']}" in starting_state:
            return
        model = model_list[0]
            ...
            for index_task, task in enumerate(tasks):
                ...
                try:
                    with Open(report_label_file, "a") as f:
                        ...
                        f.write(f'{subjectid_studyid},"{skipped_report}",{task},{skipped_line_text}\n')
                finally:
                    report_label_lock.release()
        return
    import joblib
