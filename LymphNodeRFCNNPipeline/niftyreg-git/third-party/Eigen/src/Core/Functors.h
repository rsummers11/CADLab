...
template<typename Scalar, typename OtherScalar> struct scalar_binary_pow_op
{
   ...
   inline Scalar operator() (const Scalar &a, const OtherScalar &b) const
   {
...
struct scalar_boolean_and_op
{
   ...
   EIGEN_STRONG_INLINE bool operator() (const bool &a, const bool &b) const
   {
      return a && b;
   }
};
...
// FIXME move this to functor_traits adding a functor_default
template<typename Functor> struct functor_is_product_like
{
   enum { ret = 0 };
};
template<typename LhsScalar,typename RhsScalar> struct functor_is_product_like<scalar_product_op<LhsScalar,RhsScalar> >
{
   enum { ret = 1 };
};
template<typename LhsScalar,typename RhsScalar> struct functor_is_product_like<scalar_conj_product_op<LhsScalar,RhsScalar> >
{
   enum { ret = 1 };
};
template<typename LhsScalar,typename RhsScalar> struct functor_is_product_like<scalar_quotient_op<LhsScalar,RhsScalar> >
{
   enum { ret = 1 };
};
