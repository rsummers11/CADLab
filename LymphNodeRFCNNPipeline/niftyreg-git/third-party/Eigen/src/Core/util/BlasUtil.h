...
template<> struct conj_if<false>
{
   ...
   template<typename T>
   inline const T &pconj(const T &x)
   {
      return x;
   }
};
...
template<typename XprType> struct blas_traits
{
   ...
   static inline const Scalar extractScalarFactor(const XprType &)
   {
      return Scalar(1);
   }
};
...
{
   static typename T::Scalar* run(const T &)
   {
      return 0;
   }
};
