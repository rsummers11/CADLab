...
# set epoch
EPOCH_PREVIOUS=1200 # THIS NEEDS TO BE ADJUSTED ACCOURENTLY TO PREVIOUS FINAL TRAINING EPOCH
...
echo 'epoch was: ' $EPOCH_PREVIOUS 'then plus: ' $EPOCH_1 $EPOCH_2 $EPOCH_3 $EPOCH_4
...
EPOCH=`expr $EPOCH_PREVIOUS + $EPOCH_1`
$EXEC_CMD $NN_ROOT/convnet.py -f $IN_MODEL_DIR/$FILE_NAME \
    --test-only=0 \
    --data-path=$DATA_SET_PATH \
    --save-path=$MODEL_DIR \
    --test-range=5 --train-range=1-4 \
    --test-freq=20 \
    --epoch=$EPOCH --adp-drop=0 \
    --gpu=$GPU --mini=$MINI_1\
